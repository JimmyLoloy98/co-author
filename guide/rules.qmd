...
**content-standards.md** specifies how tables, figures, and PDFs should look. It covers booktabs formatting, effect-size reporting conventions (effect sizes with confidence intervals per the ACM SIGSOFT Empirical Standards), preferred packages (`pyfixest.etable` / pandas in Python, `modelsummary` / `fixest::etable` in R), figure typography (serif fonts, no in-plot titles), and the exploration folder lifecycle.
...
| INV-4 | Significance reporting follows venue profile (EMSE/ESEM/MSR: effect sizes with CIs) | Paper | writer-critic | -3 |
| INV-5 | Abstract is 150 words or fewer | Paper | writer-critic | -5 |
| INV-6 | Keywords (and ACM CCS concepts for ACM venues) present after abstract | Paper | writer-critic | -5 |
| INV-7 | Notation consistent across all sections | Paper | writer-critic | -5 |
| INV-8 | Every causal claim has a causal design + threats-to-validity discussion | Paper | writer-critic | -10 |
...
| INV-16 | No absolute paths; use `pathlib.Path` (Python) or `here()` (R) | Code | coder-critic, verifier | -5 |
...
Most paper invariants enforce standard empirical software engineering formatting that venues expect. A few deserve explanation:

**INV-4** (significance reporting) adapts to the target venue. Empirical-standards venues (EMSE, ESEM, MSR) expect effect sizes (Cliff's delta, Vargha-Delaney $\hat{A}_{12}$) with confidence intervals alongside p-values, per the ACM SIGSOFT Empirical Standards. Working papers default to the `* p < 0.10, ** p < 0.05, *** p < 0.01` convention with a note defining thresholds. The venue profile determines which convention applies.

**INV-7** (notation consistency) catches a common drafting problem: using $\beta$ for the treatment effect in Section 3 but $\delta$ in Section 6, or using $i$ for developers in one place and projects in another. The same symbol must mean the same thing everywhere.

**INV-8** (causal claims need a causal design) prevents the most common referee complaint in empirical software engineering. If you write "X causes Y," you need a causal design and a threats-to-validity discussion explaining how you separate causation from correlation. Descriptive or purely observational mining papers should not use causal language at all.
...
**INV-13** (bare tabular export) enforces the division of labor between code and paper. Python/R scripts produce the raw table content. The `main.tex` file wraps it with `\begin{table}`, `\caption{}`, and notes. This means you can change a caption without re-running code, and code changes automatically flow into the paper.
...
**INV-16** (no absolute paths) is the single most common portability failure. Code that works on your machine but breaks on a collaborator's because of `/Users/yourname/...` paths is not reproducible code. Use `pathlib.Path` relative to the project root in Python, or `here::here()` in R.
...
| Strategy | Constructive (medium) | The study design must be sound, but the critic should suggest alternatives, not just reject. |
...
**Theory weight** applies only to papers with formal methodological content (statistical/measurement methods, theory + empirics, metric-validation studies, methodological causal designs). For applied papers using off-the-shelf estimators, the theory row is excluded and remaining weights renormalize.
...
For submission, the aggregate score must be >= 95 **and** no individual component can be below 80. A perfect literature review cannot compensate for a broken study design. A beautiful manuscript cannot compensate for code that does not reproduce.
