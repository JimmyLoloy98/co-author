...
**Research papers have a production function: design, analyze, write, submit.** This scaffold automates each stage with specialized agents that create, review, and gate-check every artifact before it advances. One [Claude Code](https://docs.anthropic.com/en/docs/claude-code) configuration handles the full pipeline --- from literature review to venue submission.

Works for empirical software engineering out of the box. Adapts to software team management, developer productivity, DevOps, code review, CI/CD, human aspects of SE, mining software repositories, and adjacent fields (HCI, CSCW, information systems) by customizing the domain profile and venue profiles.
...
2. **Strategy** --- study design, producing a strategy memo with assumptions, pseudo-code, threats to validity, and robustness plan.
...
- "Review my paper" --- checks study design, code, writing, and LaTeX in parallel
- `/review --methods paper/main.tex` --- validates your study design assumptions, clustering, and inference
- `/analyze Data/cleaned/survey.csv` --- cleans data, runs your specification, produces tables and figures
- `/new-project code review practices` --- walks you through the full pipeline from literature review to submission
...
3. **Common data sources** and study designs
4. **Field conventions** and seminal references

**Configure permissions** --- Review `.claude/settings.json`. The template includes permissions for git, LaTeX, Python, R, and utility scripts.
...
| Python | Analysis & figures | [python.org](https://www.python.org/) |
| [gh CLI](https://cli.github.com/) | GitHub integration | `brew install gh` (macOS) |

Optional: R (secondary analysis language), [Quarto](https://quarto.org) (web slides).
