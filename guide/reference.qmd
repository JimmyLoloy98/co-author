...
| `/strategize` | `strategy`, `pap`, `pap interactive`, `theory` | Study design, pre-registration plan, or formal methods section |
...
4. Run `/strategize` to design the study. If the paper type calls for formal methodological content, also run `/strategize theory`.
...
- **Data:** Ranked dataset list with feasibility grades. Each dataset assessed on measurement validity, sample selection, external validity, study-design compatibility, and known issues. Includes a rejection table for discarded datasets.
- **Ideation:** 3--5 research questions ranked by feasibility and novelty, each with a candidate study design.
...
| Mode | Description |
|------|-------------|
| `strategy` (default) | Design the study for any supported paper type |
| `pap [spec]` | Draft pre-registration plan in OSF / registered-report format |
| `pap interactive` | Guided 6-question PAP interview, then draft |
| `theory [target]` | Formal methods section: assumptions, theorems, proofs |
...
- **Strategy:** Strategy memo (design choice, estimand, assumptions, pseudo-code, robustness plan, threats to validity, top 5 referee objections). Decision record with rejected alternatives and invalidation conditions. HTML report.
- **PAP:** Full pre-registration plan with 10 standard sections. Platform-adapted for OSF preregistration or a registered-report submission (EMSE/ESEM). ASSUMED placeholders flagged with a pre-registration checklist. Adapted for observational mining studies when applicable.
...
**What to do next:** Review the strategy memo carefully --- it becomes the contract the Coder implements. Confirm the study design, assumptions, and robustness plan. For theory mode, verify proof correctness and assumption minimality before proceeding.

---

## `/analyze`

**Syntax:** `/analyze [dataset path or goal]` or `/analyze --dual python,r`
...
| Mode | Description |
|------|-------------|
| default | Single-language analysis (Python by default, or as declared in `CLAUDE.md`) |
...
| Mode | Agents | Output Path |
|------|--------|-------------|
| default | Data-engineer + Coder + coder-critic | `scripts/python/`, `paper/tables/`, `paper/figures/`, `quality_reports/results_summary.md` |
...
- Numbered scripts in `scripts/python/` (or language-appropriate directory) following the standard layout: `00_master`, `01_setup`, `02_data_preparation`, `03_descriptive`, `04_estimation`, `05_robustness`, `06_figures`, `07_tables`.
...
- `results_summary.md` with all point estimates, standard errors, effect sizes with CIs, sample sizes, and anomaly flags. This is the mandatory handoff artifact to the Writer.
- Serialized objects for every computed result (`.pkl` / `.parquet` in Python, `.rds` in R): model fits, cleaned data, summary statistics.
...
2. Detect paper type (causal mining, controlled experiment, survey, case study, descriptive/measurement, theory+empirics) from the strategy memo.
...
| `strategy` | Study design (causal mining), methodology (experiment, survey, case study), or model + tests (theory+empirics) |
| `results` | Results narration adapted to output type (tables, event studies, subgroup analyses) |
| `conclusion` | Conclusion with type-appropriate ending |
| `data` | Data section (expanded for descriptive/measurement papers) |
| `model` | Model section (theory+empirics or metric-validation only) |
...
| *(auto `.py`/`.R`)* | Code review: coder-critic standalone (categories 4--12) |
...
| `--methods [file]` | Study-design audit only: strategist-critic 4-phase review (design, assumptions, inference, threats to validity) |
...
| `--stress` | Editor + 2 adversarial referees (hostile dispositions, 2 critical pet peeves each) | `quality_reports/reviews/` |
...
**Peer review details:** The editor reads the paper and decides DESK REJECT or SEND TO REFEREES. If sent, the editor assigns each referee a disposition (RIGOR, RELEVANCE, MEASUREMENT, GENERALIZABILITY, THEORY, or OPENSCIENCE) plus one critical and one constructive pet peeve from the venue's referee pool. Both referees review independently and blind. Every major comment must include "what would change my mind." The editor then classifies each concern as FATAL, ADDRESSABLE, or TASTE and produces an editorial decision: Accept / Minor Revisions / Major Revisions / Reject. When a venue name is provided, the entire flow calibrates to that venue's profile.
...
2. For `target`: analyze contribution and recommend venues.
3. For `package`: build an artifact-evaluation-ready replication package (ACM/IEEE artifact badge standards) with master script and documentation.
...
| Mode | Description |
|------|-------------|
| `target` | Rank 3 venues by contribution fit, methodology fit, audience, desk rejection risk |
...
| Mode | Agents | Output Path |
|------|--------|-------------|
| `target` | Orchestrator (venue selection) | `quality_reports/journal_recommendations_[date].md` |
...
- **Target:** Ranked venue list with rationale.
...
**What to do next:** For `target`, pick a venue and confirm before proceeding. For `package`, run `audit` to verify completeness. For `final`, if it passes, review the cover letter draft and submit. If it fails, fix blocking issues and re-run.
...
| `lint [file\|dir]` | Grep-based mechanical linting for Python/R against prohibited patterns. Catches `os.chdir()` / `setwd()`, absolute paths, missing seed, `T`/`F` literals, `sapply()`, and more. Severity-graded (HIGH/MEDIUM/LOW). Always advisory. |
