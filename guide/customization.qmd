...
```markdown
**Project:** [YOUR PROJECT NAME]         # e.g., "Code Review and Defect Rates"
**Institution:** [YOUR INSTITUTION]       # e.g., "Your University"
**Field:** [YOUR FIELD]                   # e.g., "Developer Productivity"
```
...
The domain profile (`.claude/references/domain-profile.md`) calibrates every agent to your research field. Empirical software engineering is the default. The same template works for adjacent fields like HCI, CSCW, or information systems --- you fill in different venues, data sources, and conventions.
...
**Adjacent subfields:** DevOps, Human Aspects of SE, Software Team Management
```

**Target Venues.** Ranked by tier. The Orchestrator uses this for venue selection; the Librarian prioritizes these in searches.

```markdown
| Tier | Venues |
|------|--------|
| Top journals | IEEE TSE, ACM TOSEM, EMSE |
| Top conferences | ICSE, FSE, ASE, ESEM, MSR |
| Strong journals | JSS, IST |
```

**Common Data Sources.** What the Explorer looks for first, and what the explorer-critic knows the quirks of (GitHub, GH Archive, SEART GHS, Stack Overflow surveys, CI logs, issue trackers, DORA reports).

**Common Study Designs.** What the Strategist considers first. Each row names a design, a typical application, and the key threat to validity to defend.

**Field Conventions.** Norms the Coder and Writer follow --- project-level clustering, effect sizes (Cliff's delta, Â12) with CIs, non-parametric tests for skewed metrics, whether an explicit threats-to-validity section is expected (it is).
...
**Methodological Foundational References.** Anchor references for the Theorist agent when building or reviewing a formal methods section. Only needed for papers with formal statistical content (statistical/measurement methods, metric-validation studies, methodological causal designs). Leave empty to fall back on the generic statistical-methods defaults baked into the theorist agent.

```markdown
| Topic | Anchor references |
|-------|------------------|
| DiD with staggered adoption (MSR causal studies) | Callaway & Sant'Anna (2021) |
| Effect sizes for tool comparisons | Vargha & Delaney (2000); Arcuri & Briand (2014) |
```
...
**Field-Specific Referee Concerns.** Common objections the domain-referee and methods-referee should watch for. Examples: "Does commit count actually measure productivity?", "External validity --- does this hold beyond open source?", "Were bots filtered and developer identities merged?"

**Quality Tolerance Thresholds.** Numerical precision standards for cross-language replication and simulation checks.

---

# Venue Profiles {#sec-journal-profiles}

The venue profiles file (`.claude/references/journal-profiles.md`) calibrates the Editor, domain-referee, and methods-referee when you run `/review --peer [venue]`.

## Anatomy of a profile entry

Each profile is a structured block of plain-language fields. Here is the IEEE TSE entry:

```markdown
### IEEE Transactions on Software Engineering (TSE)
**Focus:** All areas of software engineering --- the field's broadest journal
**Bar:** Must interest SE researchers outside your subarea. Significant
        question, rigorous execution, clear contribution over the state of the art.
**Domain referee adjusts:** Contribution must be broad. Literature
        positioning against the *general* SE frontier, not just subarea.
**Methods referee adjusts:** Study design must convince non-specialists.
        Explicit threats-to-validity section mandatory. Replication package expected.
**Typical concerns:** "Why should SE researchers outside this subarea care?"
        "Are the threats to validity honestly discussed?"
**Referee pool:** RIGOR (high), GENERALIZABILITY (medium), MEASUREMENT (medium),
        THEORY (low), RELEVANCE (low), OPENSCIENCE (medium)
```

The **Referee pool** weights determine which dispositions the simulated editor draws when selecting referees. The two referees always get different dispositions. Six dispositions exist: RIGOR, RELEVANCE, MEASUREMENT, GENERALIZABILITY, THEORY, OPENSCIENCE. See [Agents](agents.qmd) for how the editor uses these.

## Pre-populated venues

13 venues ship with profiles across journals and conferences:

| Tier | Venues |
|------|--------|
| Top journals | IEEE TSE, ACM TOSEM, EMSE |
| Strong journals | JSS, IST, IEEE Software |
| Top conferences | ICSE, FSE, ASE, ESEM, MSR, ICSME, CHASE |

For venues not in the list, referees adapt using the venue name plus your domain profile. This still works --- it is just less precise.
...
**Table format:** [optional --- only if the venue deviates from default]
```

The **Table format** field is optional. The default reports effect sizes with confidence intervals alongside p-values (ACM SIGSOFT Empirical Standards). Include it only for venues with explicit style deviations.
...
Two language-specific files live in `.claude/references/`:

| File | Language |
|------|----------|
| `coding-standards-python.md` | Python (>= 3.11) --- primary |
| `coding-standards-r.md` | R (>= 4.3.0) --- secondary |

Each file specifies:

- **Runtime version** and dependency management (`venv`/`conda`, `requirements.txt`; `renv` for R)
- **Core package stack** --- e.g., Python uses `pandas`, `statsmodels`, `linearmodels`, `pyfixest`, `matplotlib`, `pydriller`, `PyGithub`
- **Prohibited packages** --- e.g., `sklearn` for statistical inference (use `statsmodels`), `plotly` for paper figures (use `matplotlib`)
- **Naming conventions** --- files, functions, variables, constants
- **Prohibited patterns** --- `os.chdir()`, hardcoded paths, `np.random.seed()` global state, bare `except:`, mutable default arguments, growing lists in loops

The Coder agent reads the relevant standards file before writing any code. The coder-critic checks compliance and deducts points for violations. CLAUDE.md declares the project language; if none is declared, the default is Python.
