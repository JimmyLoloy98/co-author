...
flowchart LR
    D(Discovery<br/>Literature + Data):::phase1 --> S(Strategy<br/>Study Design):::phase2
...
**Simulated peer review catches fatal problems before real referees do.** One command dispatches two independent reviewers calibrated to your target venue.

**What you type:** `/review --peer EMSE paper/main.tex`

**What happens behind the scenes:** The editor does a desk review --- checks scope, assigns each referee a disposition (Rigor, Relevance, OpenScience, etc.) and pet peeves that shape their review personality. Two blind referees then review independently: a domain-referee (subject expertise) and a methods-referee (empirical methods). Neither sees the other's report.

**What you see:** An editorial decision that classifies every concern as FATAL, ADDRESSABLE, or TASTE:

```markdown
# Editorial Decision --- Empirical Software Engineering
**Decision:** Major Revisions

## Referee 1 (domain-referee) --- Disposition: Relevance
- ADDRESSABLE: Missing discussion of practitioner implications (Section 6)
- ADDRESSABLE: Literature review omits Garcia & Santos (2024) on same developer population
- TASTE: Prefer effect sizes (Cliff's delta) with CIs over bare p-values

## Referee 2 (methods-referee) --- Disposition: Rigor
- FATAL: Pre-trends test shows significant coefficient at t-2 (Figure 3)
- ADDRESSABLE: Clustering at project level with 12 projects --- use wild bootstrap
...
**What happens behind the scenes:** Claude reads your paper type (causal mining, controlled experiment, survey, case study, descriptive/measurement, or theory+empirics), your strategy memo, and any existing sections. The writer drafts using paragraph-level argument moves --- each paragraph has one job:

```
Paragraph 1 [MOTIVATION]: Big question --- why this matters for software practice
Paragraph 2 [GAP]: What the literature hasn't answered
Paragraph 3 [THIS PAPER]: "We estimate..." --- result preview with magnitude
Paragraph 4 [DESIGN]: Source of variation, one-sentence study design
Paragraph 5 [PREVIEW]: Key finding with effect size
Paragraph 6 [ROADMAP]: Section-by-section guide
```

A cleanup pass then strips 24 known AI writing patterns (hedging, filler phrases, over-signposting). The argument structure adapts to your paper type --- causal mining leads with the design and result, experiments lead with the task and treatment effect, surveys lead with the population and constructs, descriptive papers lead with the data innovation and key fact.
...
2. **Strategy** --- the strategist classifies your paper type and designs the study. The strategist-critic runs a 4-phase adversarial review. You see a strategy memo with assumptions, pseudo-code, threats to validity, and a robustness plan.
...
| CLARIFICATION | Writer | "Explain your construct validity argument more clearly" |
...
**What happens behind the scenes:** The storyteller reads your paper and builds a slide deck in the specified format (seminar: 25--35 slides, 30--45 min). It applies format-specific narrative arcs --- causal mining talks lead with the practitioner question, experiment talks walk through the task and treatment, descriptive talks open with the data innovation. The storyteller-critic reviews for overflow, content fidelity, and narrative flow.
