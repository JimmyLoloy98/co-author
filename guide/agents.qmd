...
The strategist-critic works the same way. Its identity file (62 lines) says: "You are a top-venue referee (TSE/TOSEM/EMSE). Run 4 sequential phases. Read the study-design audit template." The 4-phase audit protocol, the design-specific checklists, and the scoring rubric all live in `review/templates/` and `review/config/`. Updating how we evaluate RDD papers (say, a mining study exploiting a platform threshold) means editing a template, not touching the critic's identity.
...
The **librarian** searches top SE journals (TSE, TOSEM, EMSE), top conferences (ICSE, FSE, ASE, ESEM, MSR), preprint and index sources (arXiv cs.SE, ACM DL, IEEE Xplore, DBLP), and citation chains to produce an annotated bibliography, a BibTeX file, a frontier map (what has been done), and a positioning statement (where the new paper fits). The **explorer** searches for data sources --- repository mining (GitHub API, GH Archive, SEART GHS), surveys (Stack Overflow Developer Survey, DORA State of DevOps), CI/build telemetry (TravisTorrent), issue trackers (Jira datasets) --- and grades each by feasibility: variable coverage, access requirements, panel structure, and known data issues.

Both run in parallel from the start of a project. Neither depends on the other. The librarian-critic checks for coverage gaps (missing seminal papers, missing methods literature, over-reliance on preprints). The explorer-critic checks measurement validity (does the mined metric actually capture the construct?) and study-design compatibility (does this data support the likely design?).
...
The **strategist** is paper-type aware. Given the literature and data assessment, it classifies the paper type (causal mining, controlled experiment, survey, case study, descriptive/measurement, theory+empirics) and applies the corresponding strategy template. For causal mining studies, it assesses the design landscape, proposes designs ranked by credibility, and specifies design-specific estimation guidance (modern DiD estimators for staggered CI adoption, effective F for IV, MSE-optimal bandwidth for RDD around platform thresholds). For experiments and surveys, it specifies the constructs, instruments, sampling frame, power analysis, and analysis plan. It always anticipates the top 5 referee objections.
...
The **strategist-critic** is the system's most important gatekeeper. It runs a 4-phase sequential audit: identify the design, check whether the core assumptions hold (design-specific checklists, threats to validity), evaluate inference and code-theory alignment, then check robustness. Critical finding in Phase 2 stops the review --- no point checking robustness if parallel trends are violated.
...
The **coder** translates the strategy memo into numbered analysis scripts (`00_master.py` through `07_tables.py`) with engineering discipline: numerical guards, pre-allocation, a single seed, `pathlib.Path` (or `here()` in R) for all paths, and a paper-to-code naming map linking every $\beta$ and $Y_{pt}$ to its code variable. It implements the main specification, all robustness checks from the memo, and exports bare `tabular` environments for tables.
...
The **verifier** checks mechanical correctness in two modes. Standard mode (between phases): LaTeX compilation, script execution, file integrity, output freshness. Submission mode (before artifact deposit): package inventory, dependency verification, data provenance, end-to-end execution, output cross-reference, and artifact-evaluation README completeness (ACM/IEEE badge standards). It scores binary --- 0 or 100, contributing 5% weight to the overall project score.
...
| explorer-critic | Discovery | Checks measurement validity, sample selection, design fit | explorer |
| strategist | Strategy | Strategy memo, pseudo-code, robustness plan | strategist-critic |
| strategist-critic | Strategy | Checks assumptions, inference, robustness (all paper types) | strategist |
...
| methods-referee | Peer Review | Blind review (methods: design, analysis, inference, robustness) | None |
...
| verifier | Submission | Compilation, execution, file integrity, artifact audit | None |
