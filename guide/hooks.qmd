...
2. Filters by extension --- only runs on `.py` and `.R` files
...
| MEDIUM | Mutable default arguments | `None` default + create inside |
...
```bash
#!/bin/bash
# pre-commit-lint.sh --- Lint all staged Python/R files before commit
